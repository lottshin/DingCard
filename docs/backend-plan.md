...
Compose 的环境变量契约如下。复制模板、生成密钥、绑定域名和配置 HTTPS 的实际操作见[Docker 部署与维护](deployment.md)，这里不重复维护部署命令。
...
Compose 的容器拓扑、端口绑定、HTTPS 反代、备份恢复、升级和卸载步骤统一见[Docker 部署与维护](deployment.md)。实现上要注意三点：容器内 Nginx 只提供 HTTP；正式环境的 `WEB_PORT` 应绑定 `127.0.0.1:8080`；`db` 与 `uploads` 两个命名卷必须一起备份。这里的章节只说明实现取舍，不作为部署操作手册。
