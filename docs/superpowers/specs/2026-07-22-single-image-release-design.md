...
最终镜像使用多阶段构建。根目录 `.dockerignore` 必须允许 `server/` 进入根构建上下文，同时继续排除 `node_modules`、测试产物、文档和本地密钥：

1. 前端构建阶段在根目录执行 `npm ci` 和 `npm run build`，生成 `dist`；保留 `ARG VITE_API_BASE=/` 并在构建时注入，确保完整镜像默认使用同源 RemoteStore，而不是退回 LocalStore。
2. 后端依赖阶段在 `server` 目录执行 `npm ci --omit=dev`。
3. 最终阶段基于 `node:20-slim`，复制后端源码、生产依赖和前端 `dist`，以镜像内置的非 root `node` 用户运行。

镜像默认环境必须明确设置 `NODE_ENV=production`、`HOST=0.0.0.0`、`PORT=3000`、`DATA_DIR=/data` 和 `WEB_ROOT=/app/dist`。生产镜像启动前检查 `/app/dist/index.html` 存在；缺失时打印稳定错误并退出。宿主机直接运行后端的 API-only 模式仍允许 `WEB_ROOT` 为空，因此检查只在镜像通过 `DINGCARD_IMAGE=1` 标记的路径启用，不破坏现有 systemd/API-only 使用方式。

Fastify 保持 `/api/*` 和 `/uploads/*` 现有行为，并新增前端静态站点：

- `/assets/*` 返回带哈希资源并设置长期不可变缓存。
- 已存在的静态文件按原路径返回。
- 非 `/api`、非 `/uploads` 的 `GET`/`HEAD` 页面请求在 `Accept` 包含 `text/html` 时回退到 `index.html`，支持 SPA 路由和刷新。
- 未知 API、上传路径和非 HTML 请求保持 404，不能被 SPA 回退掩盖。
- `/api`、`/api/`、`/api/*`，以及由 `UPLOADS_PUBLIC_PATH` 配置的上传根路径、本身尾斜杠和所有子路径，都是精确排除前缀，不接受 SPA 回退；默认上传根路径为 `/uploads`。
- 根页面和 `index.html` 不使用长期缓存，以免升级后继续加载旧入口。
- `MAX_UPLOAD_BYTES` 继续由 Fastify `bodyLimit` 与 multipart 限制统一执行。

为便于测试，应用装配与进程监听分离：装配函数接收配置和依赖，入口文件只负责创建应用、监听以及启动失败处理。静态站点注册拆成独立插件，边界输入和回退行为通过 Fastify `inject` 测试。现有后端集成测试中读取 `deploy/nginx.conf` 的断言改为读取 Fastify 静态路由契约。

## Compose 与数据兼容

Compose 改为单个 `app` 服务，对外仍映射 `${WEB_PORT:-8080}`，容器内端口改为 `3000`。服务显式传入 `NODE_ENV=production`、`DINGCARD_IMAGE=1`、`HOST=0.0.0.0`、`PORT=3000`、`DATA_DIR=/data` 和 `WEB_ROOT=/app/dist`，同时声明：
...
- 仅接受 `vMAJOR.MINOR.PATCH` 和对应可选预发布后缀；其他 `v*` 标签在 metadata 步骤明确失败，不发布半成品。
- 发布后检查 manifest 同时包含两个架构。
- 发布后先用未登录 HTTP manifest 请求确认 GHCR 包已公开，再分别以 `linux/amd64` 和 `linux/arm64` 启动镜像，验证 `/`、`/api/health` 和静态资源；清理测试容器和卷。匿名检查失败或任一架构未启动时，工作流失败。
...
- `docs/deployment.md`、`.env.example`、`deploy/compose-smoke.sh`、`docs/backend-plan.md` 和后端集成测试同步移除 `web`/`server`/Nginx 运行时假设；卷备份改为定位 `app` 容器，冒烟改为验证同一容器内的 Fastify 静态资源与 API。
- 技术栈移除生产 Nginx 描述；保留 Vercel 纯前端部署说明。

## 错误与兼容边界

- `WEB_ROOT` 缺失或不是目录时，开发模式允许仅启动 API；生产镜像必须在启动时给出稳定错误并退出，防止发布一个只有 API 的残缺容器。
- `/api`、`/api/`、`/api/*`，以及当前 `UPLOADS_PUBLIC_PATH` 对应的根路径、尾斜杠和子路径永远不会回退到 `index.html`。
- 前端静态文件不存在时，只有 `GET`/`HEAD` 且 `Accept` 头包含 `text/html` 的页面导航请求回退；其他方法、缺少 HTML Accept 的请求和未知上传路径返回 404。
- `DINGCARD_IMAGE=1` 且 `WEB_ROOT` 缺失、不可读或缺少 `index.html` 时，启动以稳定错误退出；未设置该标记的 API-only 生产运行不受影响。
...
3. `docker compose up -d` 只启动一个应用容器，显式监听 `0.0.0.0:3000`，已有两个卷保持原名和挂载路径。
