...
| Compose cleanup | PASS | `dingcard-release-d0bb7c3-1784720959491` 的容器、网络、卷以及 smoke 镜像标签经独立查询均不存在。 |
...
- 单容器 smoke 使用唯一项目 `dingcard-release-d0bb7c3-1784720959491` 和空闲端口 10655；首页与 `/api/health` 首轮即 200，注册、上传、Fastify 图片直出和动态提取的 hash 资源均通过。运行命令没有引用旧 `dinka-smoke` 项目。
